...
{
    public class DbMovieRepository
    {
        private BoolflixDbContext db;
        public DbMovieRepository(BoolflixDbContext _db)
        {
            db = _db;
        }
        ...
        public void Create(FormMovie formMovie)
        {
            formMovie.Movie.Categories = new List<Category>();
            formMovie.Movie.Actors = new List<Actor>();
            if (formMovie.SelectedCategories != null)
            {
                foreach (int categoryId in formMovie.SelectedCategories)
                {
                    ...
                    formMovie.Movie.Categories.Add(selectedCategory);
                }
            }
            if (formMovie.SelectedActors != null)
            {
                foreach (int actorId in formMovie.SelectedActors)
                {
                    ...
                    formMovie.Movie.Actors.Add(SelectedActor);
                }
            }
            db.Add(formMovie.Movie);
            db.SaveChanges();
        }
